...
const CANNABIS_IMAGE_PRESETS = [
  { name: '💜 โลโก้ PURPLE PUFF', url: '/uploads/store_logo_purple_heart.jpg' },
  { name: '💜 ตราสัญลักษณ์ทางการ', url: '/logo.png' }
];
...
export const AdminProductsView: React.FC<AdminProductsViewProps> = ({
  ...
  // Helper to resize/compress image to lightweight Base64 Data URL (max width 300px, quality 0.5)
  const processImageFile = (file: File): Promise<string> => {
    return new Promise((resolve, reject) => {
      ...
      reader.onload = () => {
        ...
        img.onload = () => {
          // Resize image to max 300px width while keeping aspect ratio
          const maxWidth = 300;
          let width = img.width;
          ...
          // Export as compressed JPEG with quality 0.5 (under 300px, lightweight)
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // 1. name: not empty (if empty or whitespace, assign 'สินค้าใหม่')
      const productName = formData.product_name?.trim() || 'สินค้าใหม่';
      // 2. price: always converted to number
      const productPrice = Number(formData.price) || 0;
      // 3. category: save selected category
      const categoryId = formData.category_id || categories[0]?.id || 'cat-top';
      // 4. image: save uploaded Base64 / URL
      const imageUrl = formData.image_url?.trim() || CANNABIS_IMAGE_PRESETS[0].url;

      const productId = editingProduct?.id || formData.id || `prod-${Date.now()}`;
      const now = new Date().toISOString();
      const currentCat = categories.find(c => c.id === categoryId);
      const isGen = isGeneralOrAccessory({ category_id: categoryId }, currentCat);

      const payload: Product = {
        id: productId,
        product_name: productName,
        category_id: categoryId,
        type: (formData.type || 'Hybrid') as ProductType,
        price: productPrice,
        unit: formData.unit || (isGen ? 'ชิ้น' : '1G'),
        status: (formData.status || 'AVAILABLE') as ProductStatus,
        effect_1: isGen ? '' : (formData.effect_1?.trim() || ''),
        effect_2: isGen ? '' : (formData.effect_2?.trim() || ''),
        effect_3: isGen ? '' : (formData.effect_3?.trim() || ''),
        description: formData.description?.trim() || '',
        image_url: imageUrl,
        featured: Boolean(formData.featured),
        display_order: Number(formData.display_order) || 1,
        created_at: editingProduct?.created_at || formData.created_at || now,
        updated_at: now,
        created_by: editingProduct?.created_by || currentUser?.name || 'Admin',
        updated_by: currentUser?.name || 'Admin'
      };

      // 1. Direct save to Firestore collection('products') with merge: true to protect existing fields
      await Promise.race([
        setDoc(doc(db, 'products', productId), payload, { merge: true }),
        new Promise<void>((resolve) => setTimeout(resolve, 4000))
      ]);

      // 2. Notify parent onSaveProduct for background synchronization and logging
      onSaveProduct(payload).catch(err => {
        console.warn('Background onSaveProduct notice:', err);
      });

      // 3. Reset form, close modal, and notify success
      resetForm();
      setIsModalOpen(false);

      const successMsg = editingProduct ? 'บันทึกการแก้ไขสินค้าเรียบร้อยแล้ว ✨' : 'เพิ่มสินค้าใหม่เรียบร้อยแล้ว ✨';
